    ...
    async def analyze_all_stages(
        ...
        for stage in stages:
            try:
                ...
                results[stage] = current_status.get(stage, '')
...
Твоя задача - быть максимально точным и не засорять статусы пустой информацией."""),
    ...
    def _create_standard_block_prompt(
        ...
        prompt = f"""Ты - эксперт по анализу проектных коммуникаций. Твоя задача - извлечь ТОЧНЫЙ статус этапа из переписки.
...
Прочитай ВСЮ переписку внимательно и найди ПОСЛЕДНЮЮ актуальную информацию по этапу "{block_name_ru}".
...
2. ✅ ПРАВИЛО "ИНФОРМАЦИЯ ОТСУТСТВУЕТ":
   Если этап "{block_name_ru}" НЕ обсуждается в переписке - верни РОВНО 3 слова:
   "информация отсутствует"

   ⚠️ НО ПЕРЕД ЭТИМ:
   - Проверь ВСЕ сообщения, включая старые
   - Ищи синонимы: "монтаж"="склейка"="edit", "музыка"="трек"="саунд"
   - Ищи косвенные упоминания (для "кастинг" подходит "актеры", "съемка с детьми")
   - Проверь упоминания дат/дедлайнов связанных с этим этапом

3. ✅ ЕСЛИ ЭТАП ОБСУЖДАЕТСЯ:
   ...
   - Живой язык: "ждем ОС", "окнули", "на проверке"
...
❌ "Этап 'монтаж' не обсуждается в данной переписке"
...
"""
    ...
    def _create_custom_block_prompt(
        ...
        prompt = f"""Ты - эксперт по анализу проектных коммуникаций. Твоя задача - извлечь ТОЧНЫЙ статус задачи из переписки.
...
Прочитай ВСЮ переписку внимательно и найди ПОСЛЕДНЮЮ актуальную информацию по задаче "{block_name}".
Сфокусируйся на СВЕЖИХ сообщениях (в начале переписки) - они важнее старых.

⚠️ ВАЖНО: Ищи упоминания задачи по:
- Полному названию: "{block_name}"
- Частям названия (например: "Подбор музыки" → ищи "музыка", "подбор", "трек")
- Синонимам и смыслу (например: "выбор локации" = "локации", "съемочная площадка")
...
2. ✅ ПРАВИЛО "ИНФОРМАЦИЯ ОТСУТСТВУЕТ":
   Если задача "{block_name}" НЕ обсуждается в переписке - верни РОВНО 3 слова:
   "информация отсутствует"

   НО ПЕРЕД ЭТИМ:
   - Проверь ВСЕ сообщения, включая старые
   - Ищи части названия задачи
   - Ищи синонимы и косвенные упоминания
   - Проверь упоминания дат/дедлайнов связанных с этой задачей

3. ✅ ЕСЛИ ЗАДАЧА ОБСУЖДАЕТСЯ:
   - Напиши ТОЛЬКО факты из последних сообщений
   - Максимум 1-2 предложения
   - Укажи КОНКРЕТИКУ: что ждем, от кого, когда
   - Используй даты из переписки: "24.12", "завтра", "к вечеру"
...
ПРИМЕР 1 - НЕТ упоминаний:
Переписка: Обсуждают только локации и договор
Задача: "Подбор музыки"
✅ Ответ: информация отсутствует

ПРИМЕР 2 - Ждем согласования:
Переписка: "[Продюсер]: Отправили референсы 24.12
            [Клиент]: Спасибо, посмотрю после праздников 12-13 января"
Задача: "Согласование референсов"
✅ Ответ: Отправили 24.12, ждем обратную связь от клиента (согласование перенесено на 12-13 января)

ПРИМЕР 3 - Согласовано:
Переписка: "[Клиент]: Посмотрел варианты, супер! Берем первый вариант"
Задача: "Выбор варианта анимации"
✅ Ответ: Согласовано клиентом, выбран первый вариант

ПРИМЕР 4 - В работе:
Переписка: "[Дизайнер]: Начал работу над макетом, сегодня будет драфт к 18:00"
Задача: "Разработка макета"
✅ Ответ: В работе, драфт сегодня к 18:00

ПРИМЕР 5 - Косвенное упоминание:
Переписка: "[Команда]: Трек подобрали, отправим завтра утром"
Задача: "Подбор музыки"
✅ Ответ: Музыка подобрана, отправка завтра утром

═══════════════════════════════════════════
❌ ПРИМЕРЫ НЕПРАВИЛЬНЫХ ОТВЕТОВ
═══════════════════════════════════════════

❌ "Задача не обсуждается в данной переписке"
✅ Правильно: "информация отсутствует"
