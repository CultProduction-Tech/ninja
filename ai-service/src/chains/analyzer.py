...
class StatusAnalyzer:
    def __init__(self):
        ...
        # Separate LLM for answering user questions — needs more tokens than classify/resolve
...
Если новых терминов нет — верни пустой массив: []"""
    ...
    async def analyze_dynamic_blocks(
        ...
        # Generate descriptions for custom blocks that aren't in block_descriptions
        def _get_block_description(b: dict, all_blocks: list) -> str:
            if b['name'] in block_descriptions:
                return block_descriptions[b['name']]
            display_lower = b['display'].lower()
            # Episode/выпуск blocks — strict scoping
            if any(w in display_lower for w in ['выпуск', 'эпизод', 'серия', 'episode']):
                other_episodes = [ob['display'] for ob in all_blocks if ob != b and any(
                    w in ob['display'].lower() for w in ['выпуск', 'эпизод', 'серия', 'episode']
                )]
                warning = f" НЕ путай с {', '.join(other_episodes)}." if other_episodes else ""
                return f'ТОЛЬКО про {b["display"]}. Пиши инфо КОНКРЕТНО про эти выпуски (монтаж, правки, ОС).{warning} Рыбы — отдельный блок!'
            # Рыбы/drafts blocks
            if any(w in display_lower for w in ['рыб', 'драфт', 'draft', 'черновик']):
                return f'ТОЛЬКО рыбы/черновики ({b["display"]}). НЕ путай с финальным монтажом выпусков — это разные вещи.'
            # Графический пакет
            if any(w in display_lower for w in ['граф', 'graphic', 'дизайн', 'оформлен']):
                return f'ТОЛЬКО графическое оформление: заставки, плашки, шрифты, цвета, дисклеймеры, подписи. НЕ монтаж, НЕ рыбы.'
            # Заставка
            if any(w in display_lower for w in ['заставк', 'intro', 'opener']):
                return f'ТОЛЬКО заставка/интро. НЕ путай с графическим пакетом целиком.'
            # Default for custom blocks
            return f'Кастомный блок "{b["display"]}". Пиши ТОЛЬКО то, что явно относится к "{b["display"]}".'

        blocks_list = "\n".join(
            f"- {b['display']}: {_get_block_description(b, block_info)}" for b in block_info
        )
        expected_keys = ", ".join(f'"{b["display"]}"' for b in block_info)

        glossary_text = self._build_glossary_section()

        prompt = f"""ПЕРЕПИСКА ПРОЕКТА "{project_name}" (от СТАРЫХ к новым, ПОСЛЕДНИЕ сообщения = САМЫЕ СВЕЖИЕ):
{conversation}
...
ПРАВИЛА:
1. Переписка от СТАРЫХ к НОВЫМ. Сообщения В КОНЦЕ — САМЫЕ СВЕЖИЕ и имеют НАИВЫСШИЙ приоритет.
...
12. РАЗДЕЛЯЙ ПОХОЖИЕ БЛОКИ: "Рыбы" (черновой монтаж) ≠ "Выпуски" (финальный монтаж). "ОС по рыбам 4-10" → в блок Рыбы, НЕ в Выпуск 5-6. "Монтаж выпусков 1-3 завтра" → в блок Выпуски, НЕ в Рыбы. Если фраза содержит слово "рыбы" → это блок Рыбы. Если фраза про финальный монтаж/выпуск → это блок Выпуски.
...
5. Переписка от СТАРЫХ к НОВЫМ. Последние сообщения = самые свежие.
...
ПЕРЕПИСКА (от СТАРЫХ к новым, ПОСЛЕДНИЕ = СВЕЖИЕ):
{conversation}

ЗАДАЧА: Определи ТЕКУЩИЙ статус блока "{display_name}".

ПРАВИЛА:
1. Переписка от СТАРЫХ к НОВЫМ. Сообщения В КОНЦЕ — САМЫЕ СВЕЖИЕ. Бери инфо из ПОСЛЕДНИХ (нижних) сообщений.
...
3. ⚡ ПРИОРИТЕТ СВЕЖЕМУ: Переписка идёт от старых сообщений к новым. ВСЕГДА ориентируйся на САМЫЕ ПОСЛЕДНИЕ сообщения. Если старое сообщение говорит одно, а новое — другое, верь НОВОМУ. Особенно обращай внимание на слова "отмена", "перенос", "стоп", "правки", "изменилось" — они ОТМЕНЯЮТ предыдущую информацию.

4. ✅ ЕСЛИ ИНФОРМАЦИЯ ЕСТЬ В ПЕРЕПИСКЕ:
...
5. ✅ ЕСЛИ ИНФОРМАЦИИ НЕТ ИЛИ НЕДОСТАТОЧНО:
...
7. ✅ СПЕЦИАЛЬНЫЕ СЛУЧАИ:
   - Если вопрос про ссылки/материалы:
     ...
     • СТРОГО различай типы ссылок! Если просят "графический пакет" или "графику" — выдавай только ссылки, которые относятся к дизайну/графике/оформлению. Если просят "рыбы" или "драфты" — выдавай только черновики. Если ссылка называется "Рыбы", а просят графику — НЕ ВЫДАВАЙ эту ссылку, отвечай что графики нет.
     • Если ссылок несколько версий (черновик 1, черновик 2, финал) — отдавай приоритет ФИНАЛЬНЫМ и САМЫМ ПОСЛЕДНИМ по дате.
     • Если просят просто "скинь ссылку" — дай ссылку и короткое название, без лишних цитат и пересказа переписки.
   - Если вопрос про людей - ищи имена, никнеймы, упоминания
   - Если вопрос про даты - ищи даты, числа, слова "завтра", "послезавтра", etc

8. ⛔ НЕ ДУБЛИРУЙ абзацы и цитаты по кругу. Отвечай кратко, не повторяй одно и то же.
...
═══════════════════════════════════════════
📌 АЛГОРИТМ ОТВЕТА (следуй по шагам):
═══════════════════════════════════════════

ШАГ 1: Определи ТИП запроса:
   - ССЫЛКА (просят ссылку/материал/файл) → ШАГ 2А
   - ФАКТ (просят дату/имя/статус/решение) → ШАГ 2Б
   - ОБЩИЙ ВОПРОС (что по X?) → ШАГ 2Б

ШАГ 2А (ссылка): Найди в переписке сообщение, где РЯДОМ со ссылкой написано ИМЕННО то, что просят. "Графика" ≠ "рыбы" ≠ "аниматик". Если точного совпадения нет — отвечай что ссылки нет.

ШАГ 2Б (факт): Найди САМОЕ СВЕЖЕЕ сообщение по теме. Проверь: не было ли ПОСЛЕ него отмены/переноса/правок.

ШАГ 3: Напиши ответ ОДИН РАЗ и остановись.
...
"""
        ...
        try:
            response = await self.qa_llm.ainvoke(prompt)
