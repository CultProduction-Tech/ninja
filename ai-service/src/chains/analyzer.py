...
class StatusAnalyzer:
    def __init__(self):
        ...
        self.llm = ChatOpenAI(
            ...
            temperature=0.0,  # Минимум для точного следования инструкциям
        )
    ...
    def _create_standard_block_prompt(
        ...
        prompt = f"""Ты - аналитик проектных статусов. Твоя задача: проанализировать переписку и определить ТОЛЬКО то, что РЕАЛЬНО обсуждается.

АНАЛИЗИРУЕМЫЙ ЭТАП: "{block_name_ru}"
...
2. ⛔ СТРОГО ЗАПРЕЩЕНО писать объяснения почему нет информации
3. ⛔ СТРОГО ЗАПРЕЩЕНО считать что-то "согласованным" если:
   - Этап вообще не упоминается в переписке
   - Клиент сказал "посмотрю позже" / "перенесем"
   - Команда ждет обратную связь

4. ✅ ПРАВИЛО "ИНФОРМАЦИЯ ОТСУТСТВУЕТ":
   Если этап "{block_name_ru}" НЕ упоминается в переписке напрямую -
   верни РОВНО 3 слова: "информация отсутствует"
   БЕЗ объяснений, БЕЗ дополнительного текста!

5. ✅ ПРАВИЛО "КРАТКИЙ СТАТУС":
   Если этап обсуждается - макс 2 предложения, без воды
...
НЕ СОГЛАСОВАНО:
❌ "Отправили, ждем ОС от клиента"
❌ "Клиент сказал посмотрит позже"
❌ "Клиент перенес на 12-13 января"
❌ "На согласовании у клиента"

СОГЛАСОВАНО (только эти случаи):
✅ "Клиент одобрил"
✅ "Клиент сказал окей/супер и дал добро"
✅ "Получили финальное утверждение"

═══════════════════════════════════════════
✅ ПРИМЕРЫ ПРАВИЛЬНЫХ ОТВЕТОВ
═══════════════════════════════════════════

ПРИМЕР 1 - Этап НЕ упоминается:
Переписка: Обсуждают локации и кастинг
Ответ для этапа "монтаж": информация отсутствует

ПРИМЕР 2 - Ждем согласования:
Переписка: "Отправили статику 24.12, ждем ОС от клиента. Клиент сказал посмотрит после праздников, примерно 12-13 января"
Ответ: Отправили статику 24.12, ждем обратную связь от клиента, согласование перенесено на 12-13 января

ПРИМЕР 3 - Согласовано:
Переписка: "Клиент посмотрел стайлшоты и сказал супер, можно продолжать"
Ответ: Согласовано клиентом

═══════════════════════════════════════════
⚠️ НЕПРАВИЛЬНЫЕ ОТВЕТЫ (НЕ ДЕЛАЙ ТАК!)
═══════════════════════════════════════════

❌ "Задача 'монтаж' не упоминается в переписке"
✅ Правильно: "информация отсутствует"

❌ "Этап упоминается в пайплайне но не обсуждается детально..."
✅ Правильно: "информация отсутствует"
...
ТЕПЕРЬ ПРОАНАЛИЗИРУЙ ПЕРЕПИСКУ ДЛЯ ЭТАПА "{block_name_ru}":
"""
...
АНАЛИЗИРУЕМАЯ ЗАДАЧА: "{block_name}"
...
2. ⛔ СТРОГО ЗАПРЕЩЕНО писать объяснения почему нет информации
3. ⛔ СТРОГО ЗАПРЕЩЕНО считать что-то "согласованным" если:
   - Задача вообще не упоминается в переписке
   - Клиент сказал "посмотрю позже" / "перенесем"
   - Команда ждет обратную связь

4. ✅ ПРАВИЛО "ИНФОРМАЦИЯ ОТСУТСТВУЕТ":
   Если задача "{block_name}" НЕ упоминается в переписке напрямую -
   верни РОВНО 3 слова: "информация отсутствует"
   БЕЗ объяснений, БЕЗ дополнительного текста!

5. ✅ ПРАВИЛО "КРАТКИЙ СТАТУС":
   Если задача обсуждается - макс 2 предложения, без воды
...
ПРИМЕР 1 - Задача НЕ упоминается:
Переписка: Обсуждают локации и кастинг
Ответ для задачи "подбор музыки": информация отсутствует

ПРИМЕР 2 - Ждем согласования:
Переписка: "Отправили материалы 24.12, ждем ОС от клиента. Клиент сказал посмотрит после праздников, примерно 12-13 января"
Ответ: Отправили материалы 24.12, ждем обратную связь от клиента, согласование перенесено на 12-13 января

ПРИМЕР 3 - Согласовано:
Переписка: "Клиент посмотрел и сказал супер, можно продолжать"
Ответ: Согласовано клиентом
...
❌ "Задача 'подбор музыки' не упоминается в переписке"
✅ Правильно: "информация отсутствует"

❌ "Задача упоминается в пайплайне но не обсуждается детально..."
✅ Правильно: "информация отсутствует"
...
ТЕПЕРЬ ПРОАНАЛИЗИРУЙ ПЕРЕПИСКУ ДЛЯ ЗАДАЧИ "{block_name}":
"""
